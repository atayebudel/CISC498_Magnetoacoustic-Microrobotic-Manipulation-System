...
// new serial baud rate of 500000
#include <AD9850.h>
#include <Wire.h>
...
//Store current data to send to python
struct current_data {
...
ACS712 sensor1(A0, vcc, adcMax, sens); //Coil 1 : +Y Brown
ACS712 sensor2(A1, vcc, adcMax, sens); //Coil 2 : +X Purple
ACS712 sensor3(A2, vcc, adcMax, sens); //Coil 3 : -Y Green
ACS712 sensor4(A3, vcc, adcMax, sens); //Coil 4: -X Blue
ACS712 sensor5(A4, vcc, adcMax, sens); //Coil 5 : +Z  Yellow
ACS712 sensor6(A5, vcc, adcMax, sens); //Coil 6 : -Z Orange

float readCurrent(ACS712& sensor) {
    return sensor.mA_DC() / 1000.0;  // one fast read
}
...
void setup()
{
  ...
  // center ACS712 current sensorss
...
void loop()
{
  ...
  // if gradient status = 1: output the the corresponding gradient field
   ...
   // if gradient status = 0: output the correspending uniform field
    ...
    // read current 1 sensor. adjust sign for direction of PWM
    float I1 = readCurrent(sensor1);
    if (analogRead(Coil1_PWML) > 0) I1 = -I1;
    Current_Data.Coil1_current = I1;

    // read current 2 sensor. adjust sign for direction of PWM
    float I2 = readCurrent(sensor2);
    if (analogRead(Coil2_PWML) > 0) I2 = -I2;
    Current_Data.Coil2_current = I2;

    // read current 3 sensor. adjust sign for direction of PWM
    float I3 = readCurrent(sensor3);
    if (analogRead(Coil3_PWML) > 0) I3 = -I3;
    Current_Data.Coil3_current = I3;

    // read current 4 sensor. adjust sign for direction of PWM
    float I4 = readCurrent(sensor4);
    if (analogRead(Coil4_PWML) > 0) I4 = -I4;
    Current_Data.Coil4_current = I4;

    // read current 6 sensor. adjust sign for direction of PWM
    float I5 = readCurrent(sensor5);
    if (analogRead(Coil5_PWML) > 0) I5 = -I5;
    Current_Data.Coil5_current = I5;

    // read current 6 sensor. adjust sign for direction of PWM
    float I6 = readCurrent(sensor6);
    if (analogRead(Coil6_PWML) > 0) I6 = -I6;
    Current_Data.Coil6_current = I6;
